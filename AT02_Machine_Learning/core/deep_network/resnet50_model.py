...
class ResNet50Model(Algorithm):
    def run(self, folder_path, word, min_percentage):
        result = []
        model = ResNet50(weights='imagenet', include_top=True)
        for img_name in os.listdir(folder_path):
            full_path = folder_path + img_name
            img1 = image.load_img(full_path, target_size=(224, 224))
            x = image.img_to_array(img1)
            x = np.expand_dims(x, axis=0)
            x = preprocess_input(x)
            prediction = model.predict(x)
            prediction_array = self.find_word(decode_predictions(prediction, top=4)[0], word, img_name, full_path, min_percentage)
